...
import { writeFileSync as _writeQR } from 'fs';
...
async function startSocket() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      ...
      console.log('\nWaiting for scan...  (also available at http://127.0.0.1:3000/qr as PNG)\n');
      // Save raw QR string for the case where the PNG endpoint or log is hard to reach
      try { _writeQR('/Users/Ezra/.hermes/state/whatsapp-qr.txt', qr); } catch (_) {}
    }
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (contextInfo?.quotedMessage) {
        ...
        } else if (qm.documentMessage?.caption) {
          quotedText = qm.documentMessage.caption;
        }
      }
...
app.get('/qr', async (req, res) => {
  if (!currentQrString) return res.status(404).type('text/plain').send('No QR active. If a previous QR was scanned and failed, restart the bridge to regenerate one.');
  try {
    const buf = await QRCodePng.toBuffer(currentQrString, { type: 'png', width: 512, margin: 2 });
    res.setHeader('Content-Type', 'image/png');
    res.setHeader('Cache-Control', 'no-store');
    res.send(buf);
  } catch (e) {
    res.status(500).type('text/plain').send('QR render failed: ' + e.message);
  }
});
app.get('/qr.txt', (req, res) => {
  if (!currentQrString) return res.status(404).type('text/plain').send('No QR active.');
  res.type('text/plain').send(currentQrString);
});
